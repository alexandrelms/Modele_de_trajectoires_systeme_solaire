...
nbMembre = 15;
nbGeneration = 40;
r=507597 + 3389.5e3; %rayon satellites sur mars


limiteTemps = 792;
deltaVlimite= 10000;
rayonTerremin = 6371e3 + 1666642;
...
for i = 1:1:nbMembre
    ...
    %jour et temps
    population(i).temps = randi([1 400]);
...
for j = 1:1:nbGeneration
    ...
    for i = 1:1:nbMembre
        ...
        %SPACESHIP
        %position
        SP.Px = Px_mars(population(i).temps,1) + r*sin(population(i).teta)*cos(population(i).phi); 
        SP.Py = Py_mars(population(i).temps,1) + r*sin(population(i).teta)*sin(population(i).phi);
        SP.Pz = Pz_mars(population(i).temps,1) + r*cos(population(i).teta);
        %vitesse
        SP.Vx = Vx_mars(population(i).temps,1) + population(i).Uteta*cos(population(i).teta)*cos(population(i).phi)- population(i).Uphi*sin(population(i).phi);
        SP.Vy = Vy_mars(population(i).temps,1) + population(i).Uteta*cos(population(i).teta)*sin(population(i).phi)+population(i).Uphi*cos(population(i).phi);
        SP.Vz = Vz_mars(population(i).temps,1) - population(i).Uteta*sin(population(i).teta);
        ...
        % Venus
        Venus.Px = Px_venus(population(i).temps,1); 
        Venus.Py = Py_venus(population(i).temps,1);
        Venus.Pz = Pz_venus(population(i).temps,1);
        Venus.Vx = Vx_venus(population(i).temps,1);
        Venus.Vy = Vy_venus(population(i).temps,1);
        Venus.Vz = Vz_venus(population(i).temps,1);

        
        %% Simulation
        
        sim('SIMU_trajectoire_vaisseau');
        [population(i).Distance_VT, index] = min(Distance_vaisseau_terre);
        population(i).temps_trajet = temps(index);
        
       
        %% Calcul de la finesse
        
        if  population(i).Distance_VT > rayonTerremin && population(i).deltaV < deltaVlimite && population(i).temps_trajet > 5200000 && min(Distance_vaisseau_sun) > 5e8 && max(Distance_vaisseau_sun)< 1.5e12
            
            diffDistance = population(i).Distance_VT - rayonTerremin;
    ...
    for i = 2:1:nbMembre-1
            ...
            population(i).teta = meilleureOrbite.teta + (rand*2-1)*0.1*2*3.14;  %variation 10% de l'ancienne solution
...
%% Visualisation sur un tracé
        ...
        % Venus
        Venus.Px = Px_venus(meilleureOrbite.temps,1); 
        Venus.Py = Py_venus(meilleureOrbite.temps,1);
        Venus.Pz = Pz_venus(meilleureOrbite.temps,1);
        Venus.Vx = Vx_venus(meilleureOrbite.temps,1);
        Venus.Vy = Vy_venus(meilleureOrbite.temps,1);
        Venus.Vz = Vz_venus(meilleureOrbite.temps,1);
